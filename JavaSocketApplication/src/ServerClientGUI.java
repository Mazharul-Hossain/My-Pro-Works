...
public class ServerClientGUI extends javax.swing.JFrame {
    ...
    private void ClientChooseButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ClientChooseButtonActionPerformed
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            inFileClient = fileChooser.getSelectedFile();
            //pass the file to the start transfer button.
            //To be added later
            //get and show the file path in the textbox
            filePath = inFileClient.getAbsolutePath();
            jTextField1.setText(filePath);
            //System.out.println("FilePath:"+filePath);
            long fileSize = inFileClient.length();
            String size = String.valueOf(fileSize);
            //jTextField2.setText(size + "bytes");

            ClientPrintMsg("Read File: " + inFileClient.getName() + "(size: " + size + "bytes)");
        }
    ...
    private void ClientStartTxButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ClientStartTxButtonActionPerformed
        // TODO add your handling code here:
        String server = ClientServerIPTextField.getText();
        int serverPort = Integer.parseInt(ClientPortTextField.getText());
        int size = Integer.parseInt(jTextField2.getText());

        if (ClientRMIRadioButton.isSelected()) {
        } else if (ClientSocketTCPRadioButton.isSelected()) {

            ClientPrintMsg("Starting TCP transfer...");
            tcpClient = new TCPClient(server, serverPort, size, inFileClient, this);
            tcpClient.start();

        } else if (ClientSocketUDPRadioButton.isSelected()) {

            ClientPrintMsg("Starting UDP transfer...");
            udpClient = new UDPClient(server, serverPort, size, inFileClient, this);
            udpClient.start();
        }
    ...
    private void ServerStartListeningButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ServerStartListeningButtonActionPerformed
        ...
        if (ServerRMIRadioButton.isSelected()) {
            //do nothing
        } else if (ServerSocketTCPRadioButton.isSelected()) {

            ServerPrintMsg("Starting TCP Server..");
            tcpServer = new TCPServer(serverPort, size, inFileClient.getName(), this);
            tcpServer.start();

        } else if (ServerSocketUDPRadioButton.isSelected()) {

            ServerPrintMsg("Starting UDP Server..");
            udpServer = new UDPServer(serverPort, size, inFileClient.getName(), this);
            udpServer.start();
        }
    ...
    //My functions
    public void setClientStartTimeTextField(String StartTime) {
        ClientStartTimeTextField.setText(StartTime);
    }

    public void setClientEndTimeTextField(String EndTime) {
        ClientEndTimeTextField.setText(EndTime);
    }

    public void setClientTransferTimeTextField(String TrnsfrTime) {
        ClientTransferTimeTextField.setText(TrnsfrTime);
    }

    public void setClientSpeedTextField(String Speed) {
        ClientSpeedTextField.setText(Speed);
    }

    // function for writing progress info in the text area of client
    public void ClientPrintMsg(String message) {
        message = ClientDetailsPrintingTextArea.getText() + "\n" + message;
        ClientDetailsPrintingTextArea.setText(message);
    }

    //function for writing progress info in textarea of server
    public void ServerPrintMsg(String message) {
        message = ServerTxrxDetailsPrintingTextArea.getText() + "\n" + message;
        ServerTxrxDetailsPrintingTextArea.setText(message);
    }
